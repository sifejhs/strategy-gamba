...
export function middleware(request: NextRequest) {
  ...
  // Sitemap & robots — rewrite to internal API routes so [lang] doesn't catch them
  if (path === "/sitemap.xml") {
    return NextResponse.rewrite(new URL("/api/sitemap-index", request.url));
  }
  if (path.startsWith("/sitemap/")) {
    const id = path.replace("/sitemap/", "");
    return NextResponse.rewrite(new URL(`/api/sitemap-segment/${id}`, request.url));
  }
...
export const config = {
  matcher: ["/", "/admin/:path*", "/google28a1bb12c03039aa.html", "/sitemap.xml", "/sitemap/:path*"],
};
